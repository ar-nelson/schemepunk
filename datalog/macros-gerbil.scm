...
(define-syntax %head
  (syntax-rules ()
    ...
    ((_ predicate (param . rest) (params ...) vars body) (identifier? #'param)
      (%head+var param predicate rest (params ... param) () vars body))
    ((_ predicate (param . rest) (params ...) vars body)
      (%head predicate rest (params ... param) vars body))))
...
(define-syntax %body
  (syntax-rules (? not = !=)
    ...
    ((_ ((= lhs rhs) . rest) head vars deps neg-deps guards) (identifier? #'lhs)
      (%atom+var lhs
                 ...
                 neg-deps))
    ((_ ((= lhs _) . _) . _)
      (syntax-error "First argument of datalog = must be a variable" lhs))
  ...
  (syntax-rules (quote)
    ...
    ((_ ((hd . tl) . rest) (params ...) vars guards . state)
      (%atom+var list-match
                 ...
                 . state))
    ((_ (param . rest) (params ...) . state) (underscore? #'param)
      (%atom rest (params ... wildcard-var) . state))
    ((_ (param . rest) (params ...) . state) (identifier? #'param)
  ...
  (syntax-rules (quote)
    ...
    ((_ lhs (hd . tl) vars guards . state)
      (%destruct ((lhs (car lhs) hd) (lhs (cdr lhs) tl))
                 ...
                 . state))
    ((_ lhs rhs . state) (identifier? #'lhs)
      (syntax-error "Datalog variable equality is not supported" (= lhs rhs)))
    ((_ lhs rhs . state)
      (%atom+constant lhs 'rhs () . state))))
  ...
  (syntax-rules (quote)
    ...
    ((_ ((x subject (hd . tl)) . rest) params vars guards . state)
      (%destruct ((x (car subject) hd) (x (cdr subject) tl) . rest)
                 ...
                 . state))
    ((_ ((x subject param) . rest) . state) (underscore? #'param)
      (%destruct rest . state))
    ((_ ((x subject param) . rest) params vars guards . state) (identifier? #'param)
      (%atom+var param
